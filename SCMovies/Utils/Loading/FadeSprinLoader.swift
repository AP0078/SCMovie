...
class FadeSprinLoader: UIView {
    var loaderColor: UIColor = .green.withAlphaComponent(0.4)
    
    override init(frame: CGRect) {
        super.init(frame: frame)
         setUpAnimation(in: self.layer, size: self.size, color: loaderColor)
    }
...
extension FadeSprinLoader {
    ...
    func circleAt(angle: CGFloat, size: CGFloat, origin: CGPoint, containerSize: CGSize, color: UIColor) -> CALayer {
        ...
        let circle = self.layerWith(size: CGSize(width: size, height: size), color: color)
        let frame = CGRect(
